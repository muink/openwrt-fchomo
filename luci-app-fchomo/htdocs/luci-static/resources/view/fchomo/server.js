...
function handleGenKey(option) {
	...
	switch (required_method) {
		...
		case '2022-blake3-aes-128-gcm':
			password = hm.generateRand('base64', 16);
			break;
		case 'aes-192-gcm':
			password = hm.generateRand('base64', 24);
			break;
		case 'aes-256-gcm':
		...
		case '2022-blake3-chacha20-poly1305':
			password = hm.generateRand('base64', 32);
			break;
		/* NONE */
		case 'none':
			password = '';
...
return view.extend({
	...
	render: function(data) {
		...
		o.datatype = 'port';
		o.validate = L.bind(hm.validateUniqueValue, this, data[0], 'server', 'port');
				...
				if (required_type.includes(type)) {
					if (type === 'shadowsocks') {
						...
						if (encmode === 'none')
							...
							return hm.validateBase64Key(24, section_id, value);
						else if (['2022-blake3-aes-256-gcm', '2022-blake3-chacha20-poly1305'].includes(encmode))
							return hm.validateBase64Key(44, section_id, value);
					}
		...
		*/
		...
		for (var i of hm.shadowsocks_encrypt_methods)
			o.value(i);
		...
		for (var i of hm.tls_cipher_suites)
			o.value(i);
		o.depends('tls', '1');
		o.optional = true;
		o.modalonly = true;
		...
		o.onclick = L.bind(hm.uploadCertificate, this, _('certificate'), 'server_publickey');
		o.modalonly = true;
		...
		o.onclick = L.bind(hm.uploadCertificate, this, _('private key'), 'server_privatekey');
		o.modalonly = true;
		/* TLS config end */
		...
		for (var i in hm.dns_strategy)
			o.value(i, hm.dns_strategy[i])
		o.modalonly = true;
